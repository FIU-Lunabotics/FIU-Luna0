...
public:
  ...
  int32_t update() {
    ...
    if(corruptData == true){
      ...
      byte possibleStartBytes[20];
      byte possibleEndBytes[20];
      int possStartByteIndex[20];
      int possEndByteIndex[20];
      int indexStart = 0;
      int indexEnd = 0;

      for(int i = 0; i < (sizeof(tempBarr));i++){
      ...
      if (sizeof(tempBarr) < PACKET_SIZE+1) {
        count += 1;
