...
private:
  ...
  byte joy_right_y;
  byte dX;
  byte dY; //CHANGE TO MATCH PYTHON 

public:
  /// Constructs a new empty PiData object
  PiData() {
    this->tank_mode = true; // starts on tank mode for logic to work
...
//  this->joy_right_x = 127;
  ...
  int32_t update() {
    ...
    if (barr[0] != 255 ||#include <Arduino.h>
      #include <CytronMotorDriver.h>
